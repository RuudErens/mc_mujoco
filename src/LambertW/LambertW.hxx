...
inline Float HalleyStep(const Float x, const Float w)
{
...
template<typename Float>
inline Float FritschStep(const Float x, const Float w)
{
...
template<typename Float>
inline Float SchroederStep(const Float x, const Float w)
{
...
struct Pade<Float, 0, 1>
{
  ...
  {
    return x
           * HORNER4(Float, x, 0.07066247420543414, 2.4326814530577687, 6.39672835731526, 4.663365025836821,
                     0.99999908757381)
...
struct Pade<Float, 0, 2>
{
  static inline Float Approximation(const Float x)
  {
    ...
    return 2
           + y
                 * HORNER3(Float, y, 0.00006979269679670452, 0.017110368846615806, 0.19338607770900237,
                           0.6666648896499793)
...
struct Pade<Float, -1, 4>
{
  static inline Float Approximation(const Float x)
  {
    return HORNER4(Float, x, -2793.4565508841197, -1987.3632221106518, 385.7992853617571, 277.2362778379572,
                   -7.840776922133643)
...
struct Pade<Float, -1, 5>
{
  ...
  {
    const Float y = log(-x);
    return -exp(HORNER3(Float, y, 0.16415668298255184, -3.334873920301941, 2.4831415860003747, 4.173424474574879)
...
struct Pade<Float, -1, 6>
{
  ...
  {
    const Float y = log(-x);
    return -exp(HORNER4(Float, y, 0.000026987243254533254, -0.007692106448267341, 0.28793461719300206,
                        -1.5267058884647018, -0.5370669268991288)
                / HORNER4(Float, y, 3.6006502104930343e-6, -0.0015552463555591487, 0.08801194682489769,
...
struct Pade<Float, -1, 7>
{
  static inline Float Approximation(const Float x)
  {
    return -1
           - sqrt(
               HORNER4(Float, x, 988.0070769375508, 1619.8111957356814, 989.2017745708083, 266.9332506485452,
                       26.875022558546036)
...
{
  typedef double d;
  return Y5((Branch<d, 0>::BranchPointExpansion<8>(x)), x < -0.367679,
...
{
  typedef double d;
  return Y7((Branch<d, -1>::BranchPointExpansion<8>(x)), x < -0.367579,
            (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Branch<d, -1>::BranchPointExpansion<4>(x))),
            x < -0.366079, (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 7>::Approximation(x))),
            x < -0.289379, (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 4>::Approximation(x))),
            x < -0.0509, (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 5>::Approximation(x))),
            x < -0.000131826, (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Pade<d, -1, 6>::Approximation(x))),
            x < -6.30957e-31, (Iterator<d, HalleyStep<d>>::Depth<1>::Recurse(x, Branch<d, -1>::LogRecursion<3>(x))));
}
